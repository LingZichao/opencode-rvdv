...
class MainSequence(Sequence):
    def generate(self, **kargs):
        ...
        # Stage 1: Force fill VIQ0 and maintain full load (3000 instructions)
        # Purpose: Fill VIQ0 to capacity (8 entries), VIQ1 remains empty
        # Construct high-density RAW dependency chain (60%) to prevent quick consumption
        # First 1000: FDIV.S (can only go to VIQ0)
        # Next 2000: FADD.S + FMUL.S mix (can go to VIQ0 or VIQ1, but dependency ensures VIQ0)
        
        # Track last destination register for dependency chain
        last_rd = 0
        
        # First 1000 FDIV.S instructions
        for i in range(1000):
            ...
            # 60% RAW dependency: make rs1 equal to previous rd
            if i > 0 and i % 5 < 3:  # 60% of instructions have RAW dependency
                # Use last_rd as rs1 to create RAW dependency
                self.genInstruction('FDIV.S##RISCV', {'rd': reg1, 'rs1': last_rd, 'rs2': reg3})
                last_rd = reg1
            else:
                # No dependency
        ...
        # Next 2000 FADD.S/FMUL.S instructions with high RAW dependency
        for i in range(2000):
            # Cycle through f0-f15 registers
            reg_idx = (i % 16)  # f0-f15
            reg1 = reg_idx
            ...
            # 60% RAW dependency: make rs1 equal to previous rd
            if i % 5 < 3:  # 60% of instructions have RAW dependency
                # Use last_rd as rs1 to create RAW dependency
                if i % 2 == 0:
                    self.genInstruction('FADD.S##RISCV', {'rd': reg1, 'rs1': last_rd, 'rs2': reg3})
                else:
                    self.genInstruction('FMUL.S##RISCV', {'rd': reg1, 'rs1': last_rd, 'rs2': reg3})
                last_rd = reg1
            else:
                # No dependency
                if i % 2 == 0:
        ...
        for i in range(500):
            ...
            # 20% RAW dependency
            if i % 5 == 0:
                # Use last_int_rd as rs1
                if i % 4 == 0:
                    self.genInstruction('ADD##RISCV', {'rd': rd, 'rs1': last_int_rd, 'rs2': rs2})
                elif i % 4 == 1:
                    self.genInstruction('SUB##RISCV', {'rd': rd, 'rs1': last_int_rd, 'rs2': rs2})
                elif i % 4 == 2:
                    self.genInstruction('AND##RISCV', {'rd': rd, 'rs1': last_int_rd, 'rs2': rs2})
                else:
                    self.genInstruction('OR##RISCV', {'rd': rd, 'rs1': last_int_rd, 'rs2': rs2})
                last_int_rd = rd
            else:
                if i % 4 == 0:
        ...
        # Stage 3: Trigger DLB redirection (1500 instructions without division)
        ...
        for i in range(1500):
            ...
            # 30% RAW dependency
            if i % 10 < 3:  # 30% of instructions have RAW dependency
                # Use last_fp_rd as rs1
                if i % 2 == 0:
                    self.genInstruction('FADD.S##RISCV', {'rd': reg1, 'rs1': last_fp_rd, 'rs2': reg3})
                else:
                    self.genInstruction('FMUL.S##RISCV', {'rd': reg1, 'rs1': last_fp_rd, 'rs2': reg3})
                last_fp_rd = reg1
            else:
                # No dependency
                if i % 2 == 0:
