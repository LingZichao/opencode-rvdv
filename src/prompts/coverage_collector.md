...
- 需要查询 BASELINE、当前任务覆盖率或测试列表时，先加载 `coverage-query`。
- 需要为某个 ISG 脚本生成/刷新仿真覆盖率时，先加载 `coverage-fetch`。
...
3. 如当前任务缺少仿真覆盖数据，按 `coverage-fetch` 运行仿真并返回 VDB/test 信息。
...
- 如果没有当前版本覆盖数据，使用 `coverage-fetch` 生成；如果已有 VDB/test，则直接用 `coverage-query` 查询，避免重复仿真。
