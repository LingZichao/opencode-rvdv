...
#define LINE_VP_MAX_BLOCKS 20
#define LINE_VP_GAP_SPLIT 8
#define MAX_LINE_BLOCKS 8192
...
static const char* obj_type_name(covdbHandle obj, covdbHandle region) {
    ...
    switch (objType) {
        ...
        case covdbBlock: return "covdbBlock";
        default: return "other";
    }
}

static void init_line_vp(LineVpInfo* info) {
    info->startLine = -1;
    info->endLine = -1;
    info->blockCount = 0;
    info->covered = 0;
...
static void add_line_block(LineVpInfo* info, const LineBlockInfo* block) {
    if (info == NULL || block == NULL) return;
    info->blockCount++;
    if (block->lineNo > 0 && (info->startLine < 0 || block->lineNo < info->startLine)) {
        info->startLine = block->lineNo;
    }
    if (block->lineNo > 0 && (info->endLine < 0 || block->lineNo > info->endLine)) {
        info->endLine = block->lineNo;
    }
    info->covered += block->covered;
    info->coverable += block->coverable;
}
...
static ModuleEntry* module_list_get_or_add(ModuleList* list, const char* full_name, const char* rtl_file) {
    ...
    if (list->items[list->count].full_name == NULL) {
        return NULL;
    }
...
static int module_entry_add_vp(ModuleEntry* entry, const char* range, int covered, int coverable) {
    int i;
    LineVpEntry* new_vps;
    int new_capacity;

    if (entry == NULL || range == NULL) return 0;

    for (i = 0; i < entry->vp_count; i++) {
        if (entry->vps[i].range != NULL && strcmp(entry->vps[i].range, range) == 0) {
    ...
    if (entry->vp_count == entry->vp_capacity) {
        ...
        new_vps = (LineVpEntry*)realloc(entry->vps, sizeof(LineVpEntry) * (size_t)new_capacity);
        if (new_vps == NULL) return 0;
        entry->vps = new_vps;
        entry->vp_capacity = new_capacity;
    }

    entry->vps[entry->vp_count].range = strdup(range);
    if (entry->vps[entry->vp_count].range == NULL) return 0;
...
static int collect_line_block_list(covdbHandle obj,
                                   covdbHandle region,
                                   covdbHandle test,
                                   LineBlockInfo* blocks,
                                   int maxBlocks,
                                   int count) {
    const char* typeName;
    covdbHandle iter;
    ...
    if (obj == NULL || blocks == NULL || count >= maxBlocks) return count;

    typeName = obj_type_name(obj, region);
    if (strcmp(typeName, "covdbBlock") == 0) {
        blocks[count].lineNo = covdb_get(obj, region, test, covdbLineNo);
        blocks[count].covered = covdb_get(obj, region, test, covdbCovered);
        blocks[count].coverable = covdb_get(obj, region, NULL, covdbCoverable);
        return count + 1;
    }

    if (strcmp(typeName, "covdbContainer") != 0) {
        return count;
    }

    iter = covdb_iterate(obj, covdbObjects);
    if (iter == NULL) return count;

    child = covdb_scan(iter);
    while (child != NULL && count < maxBlocks) {
        count = collect_line_block_list(child, region, test, blocks, maxBlocks, count);
        child = covdb_scan(iter);
    }
    return count;
}
...
static void collect_line_vps_for_instance(covdbHandle qualifiedInstance,
                                          covdbHandle test,
    ...
    while (rootObj != NULL) {
        const char* typeName = obj_type_name(rootObj, qualifiedInstance);
        if (strcmp(typeName, "covdbContainer") == 0) {
            covdbHandle childIter = covdb_iterate(rootObj, covdbObjects);
            covdbHandle child;
            LineBlockInfo blocks[MAX_LINE_BLOCKS];
            int blockCount = 0;
            int startIdx = 0;
            ...
            if (childIter != NULL) {
                ...
                while (child != NULL) {
                    blockCount = collect_line_block_list(child, qualifiedInstance, test, blocks, MAX_LINE_BLOCKS, blockCount);
                    child = covdb_scan(childIter);
                }
            }

            while (startIdx < blockCount) {
                LineVpInfo partInfo;
                int endIdx = startIdx;
                char rangeText[64];

                init_line_vp(&partInfo);
                add_line_block(&partInfo, &blocks[endIdx]);

                while (endIdx + 1 < blockCount) {
                    int gap = 0;
                    if (blocks[endIdx].lineNo > 0 && blocks[endIdx + 1].lineNo > 0) {
                        gap = blocks[endIdx + 1].lineNo - blocks[endIdx].lineNo;
                    }
                    if (partInfo.blockCount >= LINE_VP_MAX_BLOCKS) {
                        break;
                    }
                    if (gap > LINE_VP_GAP_SPLIT) {
                        break;
                    }
                    endIdx++;
                    add_line_block(&partInfo, &blocks[endIdx]);
                ...
                if (partInfo.startLine > 0 && partInfo.endLine >= partInfo.startLine) {
                    snprintf(rangeText, sizeof(rangeText), "%d-%d", partInfo.startLine, partInfo.endLine);
                    if (!module_entry_add_vp(entry, rangeText, partInfo.covered, partInfo.coverable)) {
                        return;
                    }
                }
...
static void process_definition_instances(covdbHandle definition,
    ...
    while (instance != NULL) {
        ...
        if (qualifiedInstance != NULL && instanceName != NULL) {
            collect_line_vps_for_instance(qualifiedInstance, test, instanceName, rtlFile, modules);
        }
        if (firstOnly) {
            break;
        }
...
int main(int argc, char* argv[]) {
    ...
    metric = find_metric(test, "Line");
    if (metric == NULL) {
        fprintf(stderr, "failed to find metric: Line\n");
        covdb_unload(test);
        covdb_unload(design);
        return 4;
    }
    ...
    while (definition != NULL) {
        char* moduleName = covdb_get_str(definition, covdbFullName);
        if (moduleName != NULL && (moduleFilter == NULL || strstr(moduleName, moduleFilter) != NULL)) {
