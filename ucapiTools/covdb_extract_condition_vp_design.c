...
static void module_list_free(ModuleList* list) {
    ...
    for (i = 0; i < list->count; i++) {
        ...
        for (j = 0; j < list->items[i].vp_count; j++) {
            int k;
            for (k = 0; k < list->items[i].vps[j].expr_count; k++) {
                free(list->items[i].vps[j].exprs[k]);
            }
            free(list->items[i].vps[j].exprs);
        }
...
static int module_entry_add_condition_vp(ModuleEntry* entry,
                                         const char* expr,
                                         int line,
                                         int crosses,
                                         int covered,
                                         int coverable) {
    int i;
    ConditionVpEntry* new_vps;
    int new_capacity;

    if (entry == NULL || expr == NULL || expr[0] == '\0') return 0;

    for (i = 0; i < entry->vp_count; i++) {
        if (entry->vps[i].line == line) {
            int j;
            int exprExists = 0;
            for (j = 0; j < entry->vps[i].expr_count; j++) {
                if (strcmp(entry->vps[i].exprs[j], expr) == 0) {
                    exprExists = 1;
                    break;
                }
            }
            if (!exprExists) {
                char** new_exprs;
                int expr_capacity = entry->vps[i].expr_capacity == 0 ? 4 : entry->vps[i].expr_capacity * 2;
                if (entry->vps[i].expr_count == entry->vps[i].expr_capacity) {
                    new_exprs = (char**)realloc(entry->vps[i].exprs, sizeof(char*) * (size_t)expr_capacity);
                    if (new_exprs == NULL) return 0;
                    entry->vps[i].exprs = new_exprs;
                    entry->vps[i].expr_capacity = expr_capacity;
                }
                entry->vps[i].exprs[entry->vps[i].expr_count] = strdup(expr);
                if (entry->vps[i].exprs[entry->vps[i].expr_count] == NULL) return 0;
                entry->vps[i].expr_count++;
            }
    ...
    if (entry->vp_count == entry->vp_capacity) {
        ...
        new_vps = (ConditionVpEntry*)realloc(entry->vps, sizeof(ConditionVpEntry) * (size_t)new_capacity);
    ...
    entry->vps[entry->vp_count].exprs = (char**)malloc(sizeof(char*) * 4u);
    if (entry->vps[entry->vp_count].exprs == NULL) return 0;
    entry->vps[entry->vp_count].expr_capacity = 4;
    entry->vps[entry->vp_count].expr_count = 1;
    entry->vps[entry->vp_count].exprs[0] = strdup(expr);
    if (entry->vps[entry->vp_count].exprs[0] == NULL) return 0;
    entry->vp_count++;
    return 1;
}
...
static void collect_condition_vps_for_instance(covdbHandle qualifiedInstance,
                                               covdbHandle test,
    ...
    while (rootObj != NULL) {
        if (strcmp(obj_type_name(rootObj, qualifiedInstance), "covdbContainer") == 0) {
            covdbHandle exprIter = covdb_iterate(rootObj, covdbObjects);
            covdbHandle exprObj;

            if (exprIter != NULL) {
                exprObj = covdb_scan(exprIter);
                while (exprObj != NULL) {
                    if (strcmp(obj_type_name(exprObj, qualifiedInstance), "covdbContainer") == 0) {
                        char* exprName = covdb_get_str(exprObj, covdbFullName);
                        int lineNo = covdb_get(exprObj, qualifiedInstance, test, covdbLineNo);
                        int containerCovered = covdb_get(exprObj, qualifiedInstance, test, covdbCovered);
                        int containerCoverable = covdb_get(exprObj, qualifiedInstance, NULL, covdbCoverable);
                        int crossCount = 0;
                        int crossCovered = 0;
                        int crossCoverable = 0;
                        covdbHandle crossIter = covdb_iterate(exprObj, covdbObjects);
                        ...
                        if (crossCount > 0 && exprName != NULL && exprName[0] != '\0') {
                            if (!module_entry_add_condition_vp(
                                    entry,
                                    exprName,
                                    lineNo,
                                    crossCount,
                                    crossCovered > 0 || crossCoverable > 0 ? crossCovered : containerCovered,
                                    crossCoverable > 0 ? crossCoverable : containerCoverable)) {
                                return;
                            }
                        }
                    }
...
static void process_definition_instances(covdbHandle definition,
    ...
    while (instance != NULL) {
        ...
        if (qualifiedInstance != NULL && instanceName != NULL) {
            collect_condition_vps_for_instance(qualifiedInstance, test, instanceName, rtlFile, modules);
        }
        if (firstOnly) break;
        instance = covdb_scan(iter);
    }
...
static void print_json_output(const ModuleList* modules, int firstOnly, const char* moduleFilter) {
    ...
    printf("{\n");
    printf("  \"version\": \"UCAPI_CONDITION\",\n");
    printf("  \"filters\": {\n");
    ...
    for (i = 0; i < modules->count; i++) {
        ...
        for (j = 0; j < modules->items[i].vp_count; j++) {
            ...
            printf("        {\"kind\": \"cond\", \"range\": ");
            if (modules->items[i].vps[j].line > 0) {
                char lineText[32];
                snprintf(lineText, sizeof(lineText), "%d-%d", modules->items[i].vps[j].line, modules->items[i].vps[j].line);
                json_escape_print(lineText);
            } else {
                json_escape_print("0-0");
            }
            printf(", \"expr\": ");
            if (modules->items[i].vps[j].expr_count == 1) {
                json_escape_print(modules->items[i].vps[j].exprs[0]);
            } else {
                char summary[128];
                snprintf(summary, sizeof(summary), "%d conditions @ line %d",
                         modules->items[i].vps[j].expr_count,
                         modules->items[i].vps[j].line);
                json_escape_print(summary);
            }
            printf(", \"expr_count\": %d, \"exprs\": [", modules->items[i].vps[j].expr_count);
            for (k = 0; k < modules->items[i].vps[j].expr_count; k++) {
                json_escape_print(modules->items[i].vps[j].exprs[k]);
                if (k + 1 < modules->items[i].vps[j].expr_count) printf(", ");
            }
...
int main(int argc, char* argv[]) {
    ...
    metric = find_metric(test, "Condition");
    if (metric == NULL) {
        fprintf(stderr, "failed to find metric: Condition\n");
        covdb_unload(test);
        covdb_unload(design);
        return 4;
    }
