...
static int count_children(covdbHandle obj) {
    int count = 0;
    covdbHandle iter = covdb_iterate(obj, covdbObjects);
    covdbHandle child;
...
static int module_entry_add_fsm_vp(ModuleEntry* entry,
                                   const char* name,
                                   int line,
                                   int covered,
                                   int coverable,
    ...
    if (entry == NULL || name == NULL || name[0] == '\0') return 0;

    snprintf(range_text, sizeof(range_text), "%d-%d", line > 0 ? line : 0, line > 0 ? line : 0);

    for (i = 0; i < entry->vp_count; i++) {
        if (entry->vps[i].line == line && strcmp(entry->vps[i].name, name) == 0) {
            entry->vps[i].covered = covered;
            entry->vps[i].coverable = coverable;
            entry->vps[i].children = children;
            return 1;
        }
    }

    if (entry->vp_count == entry->vp_capacity) {
        new_capacity = entry->vp_capacity == 0 ? 8 : entry->vp_capacity * 2;
        new_vps = (FsmVpEntry*)realloc(entry->vps, sizeof(FsmVpEntry) * (size_t)new_capacity);
        if (new_vps == NULL) return 0;
        entry->vps = new_vps;
        entry->vp_capacity = new_capacity;
    }

    entry->vps[entry->vp_count].name = strdup(name);
    entry->vps[entry->vp_count].range = strdup(range_text);
    if (entry->vps[entry->vp_count].name == NULL || entry->vps[entry->vp_count].range == NULL) return 0;
    entry->vps[entry->vp_count].line = line;
    entry->vps[entry->vp_count].covered = covered;
    entry->vps[entry->vp_count].coverable = coverable;
    entry->vps[entry->vp_count].children = children;
    entry->vp_count++;
    return 1;
}
...
static void collect_fsm_vps_for_instance(covdbHandle qualifiedInstance,
                                         covdbHandle test,
    ...
    while (rootObj != NULL) {
        if (strcmp(obj_type_name(rootObj, qualifiedInstance), "covdbContainer") == 0) {
            char* name = covdb_get_str(rootObj, covdbFullName);
            int lineNo = covdb_get(rootObj, qualifiedInstance, test, covdbLineNo);
            int covered = covdb_get(rootObj, qualifiedInstance, test, covdbCovered);
            int coverable = covdb_get(rootObj, qualifiedInstance, NULL, covdbCoverable);
            int children = count_children(rootObj);
            if (name != NULL && name[0] != '\0' && coverable > 0) {
                if (!module_entry_add_fsm_vp(entry, name, lineNo, covered, coverable, children)) {
                    return;
                }
            }
        }
...
static void process_definition_instances(covdbHandle definition,
    ...
    while (instance != NULL) {
        ...
        if (qualifiedInstance != NULL && instanceName != NULL) {
            collect_fsm_vps_for_instance(qualifiedInstance, test, instanceName, rtlFile, modules);
        }
        if (firstOnly) break;
        instance = covdb_scan(iter);
    }
...
static void print_json_output(const ModuleList* modules, int firstOnly, const char* moduleFilter) {
    ...
    for (i = 0; i < modules->count; i++) {
        ...
        for (j = 0; j < modules->items[i].vp_count; j++) {
            ...
            printf("        {\"kind\": \"fsm\", \"range\": ");
            json_escape_print(modules->items[i].vps[j].range);
            printf(", \"name\": ");
            json_escape_print(modules->items[i].vps[j].name);
            printf(", \"line\": %d, \"children\": %d, \"covered\": %d, \"coverable\": %d, \"pct\": %.2f}",
                   modules->items[i].vps[j].line,
                   modules->items[i].vps[j].children,
...
int main(int argc, char* argv[]) {
    ...
    metric = find_metric(test, "Fsm");
    if (metric == NULL) {
        fprintf(stderr, "failed to find metric: Fsm\n");
        covdb_unload(test);
        covdb_unload(design);
        return 4;
    }
